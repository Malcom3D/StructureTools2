...
#        self.length = 1
        ...
        for object in selection:
            if 'Load' in object.Name:
                LoadPresence = 1
                self.LinePreCalc(object.ObjectBase[0][0])
...
#        self.length = sqrt((x2-x1)**2+(y2-y1)**2+(z2-z1)**2)/1000
        print('Owner.Length: ', str(Owner.Length).split()[0])
        dist = float(str(Owner.Length).split()[0])
        self.length = round((dist/1000), 2)
    ...
    def PreDim(self, Fd, interaxis, length, fmd, fvd):
        print('PreDim Fd:', Fd)
        q = Fd*interaxis # kN/m² * m = kN/m (*1000/1000) -> N/mm
